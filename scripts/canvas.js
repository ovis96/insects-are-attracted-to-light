requirejs(["./helper"], function () {
  ...
  function Circle(x, y, radius, color, index, isMouse) {
    ...
    this.draw = () => {
      ...
      if (!this.isMouse) {
        // drawing hands/legs of insects
        c.beginPath();
        c.moveTo(this.x, this.y);

        const dist = Math.sqrt(square(dxyP.x) + square(dxyP.y));
        c.lineTo(
          this.x + (dxyP.x / dist) * radius * 3,
          this.y + (dxyP.y / dist) * radius * 3
        );
        c.stroke();
        ...
        c.moveTo(this.x, this.y);
        c.lineTo(
          this.x + (dxyN.x / dist) * radius * 3,
          this.y + (dxyN.y / dist) * radius * 3
        );
        c.stroke();
      }
    };
    ...
    this.move = () => {
      ...
      if (
        this.x + upX - this.radius < 0 ||
        this.x + upX + this.radius > window.innerWidth
      ) {
        this.dx *= -1;
      }
      if (
        this.y + upY - this.radius < 0 ||
        this.y + upY + this.radius > window.innerHeight
      ) {
        this.dy *= -1;
      }
      ...
      if (doesCollide(mouse, this)) {
        // collide with light
        const newDirection = giveMeDirection(this, mouse);
        this.color = BLACK;
        if (cannotReturn(mouse, this)) {
          // if it is inside the light speed is lower and get random direction (get mad)
        ...
        } else if (!doesCollideClosely(mouse, this) && Math.random() < 0.1) {
          // if insects collide with light, 10% can go out
  ...
  for (let i = 0; i < NUMBER_OF_CIRCLES; i++) {
    ...
    if (flag) {
      // intersected with another circle so cancel this circle find a new one
      i--;
    } else {
  ...
  setInterval(() => {
    // change direction of insects every 1 sec
    for (let i = 0; i < NUMBER_OF_CIRCLES; i++) {
      const circle = circles[i];
      if (Math.random() < 7) {
        if (canSeeLight(circle, mouse, ANGLE)) {
          // if insect can see the light center
      ...
      if (Math.random() < 0.4) {
        // give random direction make it alive
        const dxy = getRandomDirection(circles[i]);
        circles[i].dx = dxy.x;
        circles[i].dy = dxy.y;
      }
